...
btc = btc_raw[(btc_raw['Date'] >= start_date) & (btc_raw['Date'] <= end_date)].rename(columns={'Close': 'btc_close', 'Open': 'btc_open'})
...
stock = pd.merge(pd.merge(pd.merge(btc[['Date','btc_open','btc_close']], 
...
# stock.to_csv('stock_combined.csv', index=False)
